...
export class MjuLmsSsoClient {
  ...
  async getSsoForm(): Promise<SsoForm> {
    ...
    if (!action || !cRt || !publicKey) {
      throw new Error("SSO 로그인 폼 필드를 추출하지 못했습니다.");
    }
  ...
  async loginSso(userId: string, password: string): Promise<DecodedResponse> {
    ...
    const encsymka = encryptSessionKeyForSso(`${keyStr},${Date.now()}`, form.publicKey);
  ...
  async saveCourseCandidates(courses: ReturnType<typeof extractCourseCandidates>): Promise<void> {
    await fs.mkdir(path.dirname(this.config.coursesFile), { recursive: true });
    await fs.writeFile(this.config.coursesFile, JSON.stringify(courses, null, 2), "utf8");
  }
