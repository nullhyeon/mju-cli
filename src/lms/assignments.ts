...
import { STUDENT_REPORT_VIEW_URL } from "./constants.js";
import {
...
function parseSubmissionInfo(
  ...
  const status =
    normalizeText(submitForm.find(".submit_info_box .txt").first().text()) || undefined;
  const submittedAt =
    normalizeText(submitForm.find(".submit_info_box .date").first().text()) || undefined;
  const text =
    normalizeText(submitForm.find(".inner_content_wrap .content").first().text()) ||
    undefined;
...
export async function listCourseAssignments(
  ...
  const assignments = parseAssignmentListHtml(classroom.mainHtml, {
