...
export class WindowsCredentialVault implements PasswordVault {
  ...
  async savePassword(targetName: string, userName: string, password: string): Promise<void> {
  ...
  private async runPowerShell(envOverrides: Record<string, string>): Promise<string> {
